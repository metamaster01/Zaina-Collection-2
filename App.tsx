...
const defaultSiteSettings: SiteSettingsBundle = {
  storeSettings: {
    ...
    instagramUrl: "https://www.instagram.com/zaina_collection_orginal/",
    facebookUrl: "https://www.facebook.com/profile.php?id=61577092958455 ",
    twitterUrl: "#",
    logoUrl: "/logo2.png",
  ...
  seoSettings: {
    ...
    homepageDescription:
      "Discover elegant ethnic fashion at ZAINA COLLECTION. Shop new arrivals, best sellers, and more.",
...
export function App(): React.ReactElement {
  const useStickyState = <T,>(
    ...
    useEffect(() => {
      try {
        ...
        console.error(
          `Could not save state for key "${key}" to localStorage:`,
          error
        );
      }
    }, [key, value]);
  ...
  const [productForQuickShop, setProductForQuickShop] =
    useState<Product | null>(null);
  ...
  const [userRole, setUserRole] = useStickyState<UserRole>(
    null,
    "zaina-userRole"
  );
  const [currentUser, setCurrentUser] = useStickyState<
    UserProfile | AdminUser | null
  >(null, "zaina-currentUser");
  const [loginRedirectTarget, setLoginRedirectTarget] = useState<{
    page: PageName;
    data?: any;
  } | null>(null);

  // Checkout State
  const [buyNowIntent, setBuyNowIntent] = useState<CartItemForCheckout | null>(
    null
  );
  ...
  const [guidedDiscoveryPaths, setGuidedDiscoveryPaths] = useState<
    GuidedDiscoveryPath[]
  >([]);
  ...
  // Persisted UI State
  const [cartItems, setCartItems] = useStickyState<CartItem[]>(
    [],
    "zaina-cart"
  );
  ...
  useEffect(() => {
    if (isLoggedIn && currentUser) {
      ...
      const guestWishlist = JSON.parse(
        localStorage.getItem("zaina-guest-wishlist") || "[]"
      );
      const guestRecent = JSON.parse(
        localStorage.getItem("zaina-guest-recent") || "[]"
      );
  ...
  useEffect(() => {
    ...
    if (siteSettings) {
      root.style.setProperty(
        "--theme-color-primary",
        siteSettings.themeSettings.colorPrimary
      );
      root.style.setProperty(
        "--theme-color-gold",
        siteSettings.themeSettings.colorGold
      );
      root.style.setProperty(
        "--theme-color-cta-blue",
        siteSettings.themeSettings.colorCtaBlue
      );
  ...
  const handleOpenQuickShop = (product: Product) =>
    setProductForQuickShop(product);
  ...
  const addToCart = (
    ...
    setCartItems((prevItems) => {
      ...
      return [
        ...prevItems,
        { ...productWithVariantPrice, quantity, selectedVariant },
      ];
    });
  ...
  const handlePlaceOrder = async (
    order: any,
    guestDetails?: { email: string; createAccount: boolean; password?: string }
  ) => {
    try {
      const token = localStorage.getItem("zaina-authToken");
      const response = await axios.post(
    ...
    try {
      ...
      alert(
        "Failed to save category. " +
          (err.response?.data?.message || "Please check console for details.")
      );
    }
  };
  ...
  const createCmsHandler =
    (endpoint: string, itemType: string) => async (item: any) => {
      try {
        const token = localStorage.getItem("zaina-authToken");
        ...
        alert(
          `Failed to save ${itemType}. ` + (err.response?.data?.message || "")
        );
      }
    };

  const deleteCmsHandler =
    (endpoint: string, itemType: string) => async (itemId: string) => {
      try {
        const token = localStorage.getItem("zaina-authToken");
        await axios.delete(
        ...
        alert(
          `Failed to delete ${itemType}. ` + (err.response?.data?.message || "")
        );
      }
    };
  ...
  const { onSave: onSaveTestimonial, onDelete: onDeleteTestimonial } =
    createGenericCrudHandler("testimonials", "Testimonial");
  const { onSave: onSaveFaq, onDelete: onDeleteFaq } = createGenericCrudHandler(
    "faqs",
    "FAQ"
  );
  const { onSave: onSaveShippingZone } = createGenericCrudHandler(
    "shipping/zones",
    "Shipping Zone"
  );
  const { onSave: onSaveAdminUser, onDelete: onDeleteAdminUser } =
    createGenericCrudHandler("users", "Admin User");
  ...
  const onUploadMedia = async (files: File[]) => {
    for (const file of files) {
      try {
        ...
        alert(
          "Media upload failed. Please ensure your Google Cloud Storage (GCS) credentials are correctly configured in the backend .env file (google-credentials.json) as per the README."
        );
  ...
  const onDeleteMedia = async (fileId: string) => {
    ...
    try {
      ...
      alert(
        "Failed to delete media file: " + (err.response?.data?.message || "")
      );
    }
  };
  ...
  const renderPage = () => {
    switch (currentPage) {
          ...
          <>
            ...
            <ShopByOccasion
              ...
              onOccasionSelect={(name) =>
                navigateToPage("shop", { category: name })
              }
            />
            <TestimonialsSlider testimonials={testimonials} />
            <BlogPreviewSection
              posts={cmsPages.filter((p) => p.type === "post").slice(0, 3)}
      ...
      case "adminDashboard":
        return (
          <AdminDashboardPage
            ...
            onSaveShoppableVideo={createCmsHandler(
              "shoppable-videos",
              "Shoppable Video"
            )}
            onDeleteShoppableVideo={deleteCmsHandler(
              "shoppable-videos",
              "Shoppable Video"
            )}
            ...
            onSaveFloatingInfo={createCmsHandler(
              "floating-info",
              "Floating Info"
            )}
            onDeleteFloatingInfo={deleteCmsHandler(
              "floating-info",
              "Floating Info"
            )}
            onSaveFaq={onSaveFaq}
      ...
      case "blogIndex":
        return (
          <BlogIndexPage
            posts={cmsPages.filter((p) => p.type === "post")}
            navigateToPage={navigateToPage}
          />
        );
      case "blogPost":
        return pageData ? (
          <BlogPostPage post={pageData} />
        ) : (
          <NotFoundPage navigateToPage={navigateToPage} />
        );
      default:
