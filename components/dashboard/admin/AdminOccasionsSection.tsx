...
import { OccasionContent, MediaFile } from '../../../types';
...
interface AdminOccasionsSectionProps {
  occasions: OccasionContent[];
  onSave: (occasion: OccasionContent) => void;
  onDelete: (occasionId: string) => void;
  mediaLibrary: MediaFile[];
  onUploadMedia: (files: File[]) => void;
}

const AdminOccasionsSection: React.FC<AdminOccasionsSectionProps> = ({ occasions, onSave, onDelete, mediaLibrary, onUploadMedia }) => {
  const [isFormVisible, setIsFormVisible] = useState(false);
  const [editingItem, setEditingItem] = useState<OccasionContent | null>(null);
  const [formData, setFormData] = useState<Partial<OccasionContent>>({});
  ...
  const handleEdit = (item: OccasionContent) => {
    setEditingItem(item);
    setFormData(item);
  ...
  const handleAddNew = () => {
    setEditingItem(null);
    setFormData({ id: `occasion_${Date.now()}` });
    setIsFormVisible(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave(formData as OccasionContent);
    resetForm();
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold font-heading-playfair text-admin-text dark:text-admin-dark-text">Shop by Occasion</h2>
        {!isFormVisible && (
          <button onClick={handleAddNew} className="bg-admin-accent text-white font-semibold py-2 px-4 rounded-lg hover:bg-admin-accent-hover flex items-center">
            <PlusCircleIcon className="w-4 h-4 mr-2" /> Add New Occasion
          </button>
        )}
      </div>
      ...
      {isFormVisible && (
        <form onSubmit={handleSubmit} className="mb-8 p-6 bg-admin-light dark:bg-admin-dark rounded-lg shadow-inner space-y-4">
          <h3 className="text-lg font-semibold">{editingItem ? 'Edit Occasion' : 'Add New Occasion'}</h3>
          <InputField label="Occasion Name (for filtering)" name="name" value={formData.name || ''} onChange={e => setFormData(p => ({...p, name: e.target.value}))} required />
          <InputField label="Display Title" name="title" value={formData.title || ''} onChange={e => setFormData(p => ({...p, title: e.target.value}))} required />
...
export default AdminOccasionsSection;
