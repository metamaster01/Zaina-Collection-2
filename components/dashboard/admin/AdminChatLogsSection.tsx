...
const AdminChatLogsSection: React.FC = () => {
    ...
    const fetchSessions = async () => {
        try {
            ...
            const response = await axios.get(`${API_BASE_URL}/api/admin/chats`, {
    ...
    const handleSelectSession = async (session: ChatSession) => {
        ...
        try {
            const token = localStorage.getItem('zaina-authToken');
            const response = await axios.get(`${API_BASE_URL}/api/admin/chats/${session.id}/messages`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            const token = localStorage.getItem('zaina-authToken');
            await axios.post(`${API_BASE_URL}/api/admin/chats/${selectedSession.id}/messages`, 
                { text: newMessage },
                { headers: { Authorization: `Bearer ${token}` } }
            );
