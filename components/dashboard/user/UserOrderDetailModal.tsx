...
const joinVariantAttributes = (selectedVariant: any) => {
...
const UserOrderDetailModal: React.FC<UserOrderDetailModalProps> = ({ order, onClose }) => {
  ...
  // numbers sanitized
  const deliveryChargeN = numberOrZero(order.deliveryCharge);
  const totalAmountN = numberOrZero(order.totalAmount);
  const subtotalN = Math.max(0, totalAmountN - deliveryChargeN);
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between text-sm">
          <div>
            <p className="text-zaina-slate-gray dark:text-dark-zaina-text-secondary">
              Order Date:{" "}
              <span className="font-medium text-zaina-deep-navy dark:text-dark-zaina-text-primary">
                {safeDate(order.orderDate, undefined /* browser locale */)}
              </span>
            </p>
            <p className="text-zaina-slate-gray dark:text-dark-zaina-text-secondary">
              Payment Method:{" "}
            ...
            <p className="text-zaina-slate-gray dark:text-dark-zaina-text-secondary">
              Delivery Method:{" "}
              <span className="font-medium text-zaina-deep-navy dark:text-dark-zaina-text-primary capitalize">
        ...
        {/* Items */}
        <div>
          <h4 className="font-semibold text-zaina-deep-navy dark:text-dark-zaina-text-primary mb-2">
            Items
          </h4>
          <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
            {(order.items ?? []).map((item, index) => {
              const qty = numberOrZero(item?.quantity) || 1;
              const unit = numberOrZero(item?.priceAtPurchase);
              const lineTotal = unit * qty;

              const attrsText = joinVariantAttributes(item?.selectedVariant);
              ...
              return (
                <div key={index} className="flex items-center gap-4">
                  ...
                  />
                  <div className="flex-grow">
                    <p className="font-medium text-zaina-deep-navy dark:text-dark-zaina-text-primary">
                      {item?.name || "Item"}
                    </p>
                    <p className="text-xs text-zaina-slate-gray dark:text-dark-zaina-text-secondary capitalize">
                      {attrsText}
                  ...
                  <p className="font-medium text-sm text-zaina-deep-navy dark:text-dark-zaina-text-primary">
                    ₹{money(lineTotal)}
                  </p>
                </div>
              );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm pt-4 border-t border-zaina-cool-gray-light dark:border-dark-zaina-border-strong">
          <div>
            <h4 className="font-semibold text-zaina-deep-navy dark:text-dark-zaina-text-primary mb-1">
              Shipping Address
            </h4>
            <address className="not-italic text-zaina-slate-gray dark:text-dark-zaina-text-secondary">
              {(order.shippingAddress?.fullName as string) || "—"}
              <br />
              {(order.shippingAddress?.addressLine1 as string) || ""}
              <br />
          ...
          <div className="text-left sm:text-right">
            ...
            <p>Subtotal: ₹{money(subtotalN)}</p>
            <p>Delivery Charge: ₹{money(deliveryChargeN)}</p>
            <p className="font-bold text-base mt-1">Total: ₹{money(totalAmountN)}</p>
          </div>
        </div>
      </div>
    </Modal>
